...
def qblox_resonator_punchout_attenuation(
    ...
    for qname, res in node.analyses.items():
        ...
        plt.close(fig)
        plots.append(png_plot_entry(image_path, f"Resonator Punchout - {qname}"))
    ...
    if not accepted_all:
        payload["error"] = update_reason or "; ".join(
            f"{qname}: no operating point met the contrast/shift thresholds"
...
def qblox_resonator_punchout_amplitude(
    ...
    for qname, res in node.analyses.items():
        ...
        fig.savefig(image_path, dpi=160)
        plt.close(fig)
        plots.append(png_plot_entry(image_path, f"Resonator Punchout Amplitude - {qname}"))
